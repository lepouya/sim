...
export default class Resource extends React.Component {
  ...
  renderRates(
    ...
    for (let i = 0; i < rates.length; i++) {
      const rate = rates[i];
      const prefix = i > 0 ? ", " : "";
      const value = this.renderNumber(
        rate.value,
        digits,
        plus,
        origin ? rate.entity.count : 0,
        reverse,
      );
      res.push(
        <span key={i}>
          {prefix}
          {value} {rate.entity.name}
        </span>,
      );
    }
    return res;
    ...
    if (resource.requirements) {
      return (
        <div className="locked">
          Currently unavaiable; it will be unlocked when all requirements are
          met
        </div>
      );
    } else {
  ...
  renderCount(resource) {
    ...
    return (
      <div className="count">
        You currently own {count} {limit} {rate}
      </div>
    );
  }
  ...
  renderGenerator(resource) {
    ...
    if (resource.outputRate.length > 0) {
      generates = (
        <span>generates {this.renderRates(resource.outputRate)}</span>
      );
    }
    ...
    if (!consumes && !generates) {
      ...
      if (resource.genRate.length == 0) {
        ...
        currentRate = (
          <span>
            Current rate is {this.renderRates(resource.genRate, 1, true)} / s
          </span>
        );
      }
      return (
        <div className="rate">
          Each {resource.name} {consumes}
          {and}
          {generates}; {currentRate}
        </div>
      );
    }
  }
  ...
  renderBuyBox(resource) {
    ...
    const enabled = (price || []).reduce(
      (enabled, rate) => enabled && rate.entity.count >= rate.value,
      price !== undefined,
    );
    ...
    const buttonMax = resource.limit && maxEnabled && (
      <button
        onClick={(_) => this.buy(resource.limit - resource.count)}
        disabled={!enabled}
      >
        {buyMax.replace("@", "max")}
      </button>
  ...
  renderSellBox(resource) {
    ...
    const buttonOne = (
      <button onClick={(_) => this.sell(1)} disabled={!enabled}>
        {sellText.replace("@", "")} {text}
      </button>
    );
    const buttonMax = (!enabled || price.length > 0) &&
      maxEnabled &&
      resource.count > 1 && (
  ...
  render() {
    ...
    return (
      <div
        id={"resource_" + resource.name.replace(/[^a-z0-9_]/g, "_")}
        className="resource"
      >
